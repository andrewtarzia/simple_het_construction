...
from shc.utilities import AromaticCNCFactory

from .case_data import CaseData


def test_calculate_angles(case_data: CaseData) -> None:  # noqa: C901, PLR0915, PLR0912
    ...
    assert low_energy_values == case_data.low_energies
    ...
    assert min_geom_scores == case_data.min_gs

    for pair_name in pair_info:
        rdict = pair_info[pair_name]
    ...
    assert np.isclose(mean, case_data.mean_g, atol=1e-6)
    assert np.isclose(std, case_data.std_g, atol=1e-6)
